...
:::{admonition} Note
:class: note
The time quanta that stat uses is seconds; the rest of the BeOS
...
- The {hparam}`owner` identifier encodes the identity of the user that "owns" the file.
- The {hparam}`group` identifier encodes the "group" that is permitted group access to the file (as declared by the permissions).
- The {hparam}`permissions` value records nine "permission facts": Whether the file can be read, written, and executed by the node's owner, by users in the node's group, and by everybody else (read/write/execute * owner/group/others = 9 items).
...
- {cpp:enum}`S_IRUSR` owner's read bit.
- {cpp:enum}`S_IWUSR` owner's write bit.
- {cpp:enum}`S_IXUSR` owner's execute bit.
- {cpp:enum}`S_IRGRP` group's read bit.
- {cpp:enum}`S_IWGRP` group's write bit.
- {cpp:enum}`S_IXGRP` group's execute bit.
- {cpp:enum}`S_IROTH` others' read bit.
- {cpp:enum}`S_IWOTH` others' write bit.
- {cpp:enum}`S_IXOTH` others' execute bit.
...
{hmethod}`GetStat()` returns the
{ref}`stat` structure for the node. The structure is
copied into the {hparam}`st` argument, which must be allocated. The {hclass}`BStatable` object does
not cache the stat structure; every time you call {hmethod}`GetStat()`, fresh stat
