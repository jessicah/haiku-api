...
Creates a new {hclass}`BResources` object. You can initialize the object by passing
a pointer to a valid
{cpp:class}`BFile`; without the argument, the object won't refer
to a file until
{cpp:func}`~BResources::SetTo` is called.

If {hparam}`clobber` is {cpp:enum}`true`, the file
that's referred to by {cpp:class}`BFile` is truncated
(it's data is erased), and a new resource file header is written to the
file. If {hparam}`clobber` is {cpp:enum}`false` and the file doesn't otherwise doesn't have a
resource header, the initialization fails.

{hclass}`BResources` copies the
{cpp:class}`BFile`
...
- {hparam}`type` is one of the type_code constants defined in support/TypeConstants.h.
- {hparam}`id` is the ID number that you want to assign to the resource. The value of the ID has no meaning other than that which your application gives it; the only restriction on the ID is that the combination of it and the data type constant must be unique across all resources in this resource file.
- {hparam}`data` is a pointer to the data that you want the resource to hold.
- {hparam}`length` is the length of the data buffer, in bytes.
- {hparam}`name` is optional, and needn't be unique. Or even interesting.

Ownership of the data pointer isn't assigned to the {hclass}`BResources` object by
this function; after {hmethod}`AddResource()` returns, your application can free or
...
-
	...
	- The {hparam}`data` pointer is {cpp:enum}`NULL`.
-
...
Returns the {cpp:class}`BFile`
the {hclass}`BResource` object references.
...
- The first version ({hparam}`byIndex`) searches for the {hparam}`byIndex`'th resource in the file.
- The second ({hparam}`byType`/{hparam}`andIndex`) searches for the {hparam}`byIndex`'th resource that has the given type.
- The third ({hparam}`byType`/{hparam}`andId`) looks for the resource with the unique combination of type and ID.
- The fourth ({hparam}`byType`/{hparam}`andName`) looks for the first resource that has the given type and name.
- The last ({hparam}`byPointer`) returns information about the resource whose data is pointed to by {hparam}`byPointer`. This can be used to trace a resource that's already been loaded with {cpp:func}`~BResources::LoadResource` back to its information.

The other arguments return the other statistics about the resource (if
found).

The pointer that's returned in {hparam}`*foundName` belongs to the {hclass}`BResources`.
Don't free it.

The functions return {cpp:enum}`true` if a resource was found
...
Returns {cpp:enum}`true` if the resource file contains a resource as identified by
the arguments, otherwise it returns {cpp:enum}`false`.
...
hand, is unique. See
"{cpp:func}`~BResources::Identifying`."
...
resource, {cpp:enum}`NULL` is returned.
...
Copies all the resource from the file specified by {hparam}`fromFile` into the file
targeted by the {hclass}`BResources` object. The original file isn't changed. You
...
-
	- {cpp:enum}`B_NOT_ALLOWED`.
	- Couldn't find the specified resource, or an error occurred trying to remove it (first form of {cpp:func}`~BResources::RemoveResource`).
-
	- {cpp:enum}`B_ERROR`.
...
{cpp:class}`BFile`, and re-initializes it to
refer to a copy of the argument. If the new BFile is open for writing,
the {hclass}`BResources`' copy of the
{cpp:class}`BFile` is locked.

If {hparam}`clobber` is {cpp:enum}`true`, the file that's
referred to by {cpp:class}`BFile` is truncated
(it's data is erased), and a new resource file header is written to the
file. If {hparam}`clobber` is {cpp:enum}`false` and the file doesn't otherwise doesn't have a
resource header, the initialization fails.
...
very good reason not to use {hclass}`BResources` for anything other than permanent,
