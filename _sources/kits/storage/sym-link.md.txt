...
Creates a new {hclass}`BSymLink` object, initializes it according to the arguments,
and sets {cpp:func}`~BNode::InitCheck`
to return the status of the initialization.

- The default constructor does nothing and sets {cpp:func}`~BNode::InitCheck` to {cpp:enum}`B_NO_INIT`. To initialize the object, call {cpp:func}`~BNode::SetTo`.
- The copy constructor creates a new {hclass}`BSymLink` that's open on the same node as that of the argument.
- For information on the other constructors, see the analogous {cpp:func}`~BNode::SetTo` functions in the {cpp:class}`BNode` class; {hclass}`BSymLink` inherits them without change.
...
Returns {cpp:enum}`true` if the symlink contains an absolute pathname.
...
returns it as a {cpp:class}`BPath` object. For this to work you have to tell the
...
If the symlink contains an absolute path, then the {hparam}`dir` or {hparam}`dirPath`
...
-
	...
	- The object doesn't refer to a symlink, or {hparam}`dir`/{hparam}`dirPath` is {cpp:enum}`NULL`.
-
...
Copies the contents of the symlink into {hparam}`buf`. {hparam}`length` is the size of the
buffer; to be perfectly safe, the buffer should be {cpp:enum}`B_PATH_NAME_LENGTH`
...
{hmethod}`ReadLink()` can't give you a
{cpp:class}`BPath` object. If you want a
{cpp:class}`BPath` to the
linked-to entry, see
