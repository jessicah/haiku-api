...
:::{admonition} Warning
:class: warning
For {cpp:class}`BQuery`
this is a no-op. Also,
{cpp:class}`BDirectory`'s implementation
manipulates the entry list pointer; thus, you shouldn't call
{hmethod}`CountEntries()` while you're iterating through the directory's entries.
:::
::::
...
- {hmethod}`GetNextEntry()` and {hmethod}`GetNextRef()` return {cpp:enum}`B_ENTRY_NOT_FOUND`.
- {hmethod}`GetNextDirents()` returns 0.

If the {hparam}`traverse` argument is {cpp:enum}`true`, symlinks are traversed.
