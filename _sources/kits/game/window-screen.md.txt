...
:::
Initializes the {hclass}`BWindowScreen` object by
assigning the window a {hparam}`title` and specifying a
{hparam}`space` configuration for the screen. The window won't
...
{hclass}`BWindowScreen` object should establish when it
...
{cpp:func}`~set::screen`,
the {ref}`Interface Kit`
function that preference applications call to configure
the screen.
The {hparam}`space` configuration applies only
while the {hclass}`BWindowScreen` object is in
...
({cpp:enum}`B_CURRENT_WORKSPACE`). It fails if another
{hclass}`BWindowScreen` object in any
application is already assigned to the same workspace.
To be sure there wasn't an error in constructing the object, check
the {hparam}`error` argument. If all goes well, the
constructor sets the {hparam}`error` variable to
{cpp:enum}`B_OK`. If not, it sets it to
{cpp:enum}`B_ERROR`. If there's an error, it's likely to occur in
this constructor, not the inherited
{cpp:class}`BWindow` constructor.
Since the underlying window will probably exist, you'll need to instruct
it to quit. For example:
:::{code}
status_t error;
...
:::
If the {hparam}`debugging` flag is {cpp:enum}`true`, the
{hclass}`BWindowScreen` is constructed in
...
{cpp:func}`~BWindowScreen::SuspensionHook`.
The debugging regime is described under those functions.
See also: the
{cpp:class}`BScreen` class in the
{ref}`Interface Kit`
::::
...
{hclass}`BWindowScreen` object established its connection to the screen), unloads
...
This function is called with the {hparam}`connected` flag set to {cpp:enum}`true` immediately
after the {hclass}`BWindowScreen` object becomes the active window and establishes
...
{hclass}`BWindowScreen` object provides.

{hmethod}`ScreenConnected()` is called with a flag of {cpp:enum}`false` just before the
{hclass}`BWindowScreen` object is scheduled to lose its control over the screen and
the Application Server's control is reasserted. The {hclass}`BWindowScreen`'s
...
Note that whenever {hmethod}`ScreenConnected()` is called, the {hclass}`BWindowScreen` object
is guaranteed to be connected to the screen; if {hparam}`connected` is {cpp:enum}`true`, it
just became connected, if {hparam}`connected` is {cpp:enum}`false`, it's still connected but
will be disconnected when the function returns.

Derived classes typically use this function to regulate access to the
screen. For example, they may acquire a semaphore when the {hparam}`connected` flag
is {cpp:enum}`false`, so that application threads won't attempt direct drawing when
the connection isn't in place, and release the semaphore for drawing
threads to acquire when the flag is {cpp:enum}`true`. For example:
...
Returns {cpp:enum}`true` if the graphics card driver permits applications to control
the configuration of the frame buffer, and {cpp:enum}`false` if not. Control is
exercised through these two functions:

- {cpp:func}`~BWindowScreen::SetFrameBuffer`
- {cpp:func}`~BWindowScreen::MoveDisplayArea`

A return of {cpp:enum}`true` means that these functions can communicate with the
graphics card driver and at least the first will do something useful. A
return of {cpp:enum}`false` means that neither of them will work.
...
:::
Returns a pointer to the graphics card "hook" function
that's located at {hparam}`index` in its list of hook
functions. The function returns {cpp:enum}`NULL` if the graphics
...
{hmethod}`CardHookAt()`, it will return
{cpp:enum}`NULL` even if the function is implemented.
An application can cache the pointers that {hmethod}`CardHookAt()` returns, but it
should ask for a new set each time the depth or dimensions of the screen
changes and each time the {hclass}`BWindowScreen` object releases or regains
control of the screen. You'd typically call {hmethod}`CardHookAt()` in your
implementation of
...
:::
See also:
{cpp:func}`~BWindowScreen::FrameBufferInfo`
::::
...
:::
Forces the {hclass}`BWindowScreen` object to disconnect
...
{hclass}`BWindowScreen` only when hiding the game, reducing it
to an ordinary window in the background, or quitting. The
{cpp:func}`~BWindowScreen::Hide` and
{cpp:func}`~BWindowScreen::Quit`
functions automatically disconnect the {hclass}`BWindowScreen`
as part of the process of hiding and quitting.
{cpp:func}`~BWindowScreen::Disconnect`
allows you to sever the connection before calling those functions.
Before breaking the screen connection,
{cpp:func}`~BWindowScreen::Disconnect`
causes the {hclass}`BWindowScreen` object to receive a
{cpp:func}`~BWindowScreen::ScreenConnected`
notification with a flag of {cpp:enum}`false`. It doesn't return until
{cpp:func}`~BWindowScreen::ScreenConnected`
...
in addons/graphics/GraphicsCard.h
:::{admonition} Note
:class: note
...
See also:
{cpp:func}`~BWindowScreen::SetSpace`,
{cpp:func}`~BWindowScreen::SetFrameBuffer`,
{cpp:func}`~BWindowScreen::MoveDisplayArea`,
...
{cpp:class}`BWindow` counterparts to make sure that the
{hclass}`BWindowScreen` is disconnected from the screen before it's hidden and that
it's ready to establish a connection when it becomes the active window.

{hmethod}`Hide()` calls
{cpp:func}`~BWindowScreen::ScreenConnected` (with an argument of {cpp:enum}`false`) and breaks the
connection to the screen when {cpp:func}`~BWindowScreen::ScreenConnected` returns. It then hides
the window.

{hmethod}`Show()` shows the window on-screen and makes it the active window, which
will cause it to establish a direct connection to the graphics card
driver for the screen. Unlike {hmethod}`Hide()`, it may return before
{cpp:func}`~BWindowScreen::ScreenConnected`
is called (with an argument of {cpp:enum}`true`).
...
to the screen. This function moves the area's left-top corner to ({hparam}`x`, {hparam}`y`);
...
relocating the display area, this function returns {cpp:enum}`B_OK`; if not, it
returns {cpp:enum}`B_ERROR`.

See also:
{cpp:func}`~BWindowScreen::CanControlFrameBuffer`
...
Augments the {cpp:class}`BWindow`
version of {hmethod}`Quit()` to force the {hclass}`BWindowScreen` object
to disconnect itself from the screen, so that it doesn't quit while in
control of the frame buffer.

Although {hmethod}`Quit()` disconnects the object before quitting, this may not be
soon enough for your application. For example, if you need to destroy
some drawing threads before the {hclass}`BWindowScreen` object is itself destroyed,
...
{cpp:func}`~BWindowScreen::Disconnect`. For
example:
...
If the screen connection is still in place when {hmethod}`Quit()` is called, it
calls {cpp:func}`~BWindowScreen::ScreenConnected` with a flag of {cpp:enum}`false`. It doesn't return until
{cpp:func}`~BWindowScreen::ScreenConnected` returns and the connection is broken.
...
Calling {hmethod}`Suspend()` switches to the Terminal workspace programmatically,
...
activated, and the {hparam}`label` passed as an argument is displayed in a message
...
resumed—whether by the user switching workspaces or by {hmethod}`Suspend()`.
It gives you an opportunity to save and restore any state that would
otherwise be lost. {hmethod}`SuspensionHook()` is called with a {hparam}`suspended` flag of
{cpp:enum}`true` just after the application is suspended and with a flag of {cpp:enum}`false`
just before it's about to be resumed.

{hmethod}`ScreenConnected()` is not called when you switch between the Terminal and
game workspaces while in debugging mode. However, it is called for all
normal game activities—when the {hclass}`BWindowScreen` is first activated
...
Overrides the {cpp:class}`BWindow` version of {cpp:func}`~BWindow::ScreenChanged` so that it does nothing.
...
{cpp:enum}`B_CMAP8` color space). {hmethod}`SetColorList()`
is passed an array of one or more
colors to replace the colors currently in the list. The first color in
the array replaces the color in the list at the specified {hparam}`first` index;
all colors up through the {hparam}`last` specified index are modified. It fails if
either index is out of range.

{hmethod}`SetColorList()` alters the list of colors kept on the graphics card. If
the {hclass}`BWindowScreen` isn't connected to the screen, the new list takes
effect when it becomes connected.

{hmethod}`ColorList()` returns a pointer to the entire list of 256 colors. This is
not the list kept by the graphics card driver, but a local copy. It
belongs to the {hclass}`BWindowScreen` object and should be altered only by calling
{hmethod}`SetColorList()`.

See also:
{cpp:func}`BScreen::ColorMap` in the {ref}`Interface Kit`
...
Configures the frame buffer on the graphics card so that it's {hparam}`width` pixel
columns wide and {hparam}`height` pixel rows high. This function works only if the
driver for the graphics card allows custom configurations (as reported by
{cpp:func}`~BWindowScreen::CanControlFrameBuffer`) and the {hclass}`BWindowScreen` object is currently
...
{cpp:func}`~BWindowScreen::SetFrameBuffer` returns
{cpp:enum}`B_ERROR`. If the change is made, it returns {cpp:enum}`B_OK`.
...
constructor—permitted {hparam}`space` constants are documented
...
display area. For example, if {hparam}`space` is {cpp:enum}`B_32_BIT_800x600`, the frame buffer
...
{cpp:func}`~BWindowScreen::SetFrameBuffer` and
...
{cpp:enum}`SetSpace()` returns {cpp:enum}`B_ERROR`. If all goes well, it returns {cpp:enum}`B_OK`.

See also: the {hclass}`BWindowScreen`
{cpp:func}`~BWindowScreen::Constructor`,
{cpp:func}`~BWindowScreen::SetFrameBuffer`,
{cpp:func}`~BWindowScreen::MoveDisplayArea`
...
Overrides the {cpp:class}`BWindow` version of {cpp:func}`~BWindow::WindowActivated` to connect the
{hclass}`BWindowScreen` object to the screen (give it control over the graphics
card driver) when the {hparam}`active` flag is {cpp:enum}`true`.

This function doesn't disconnect the {hclass}`BWindowScreen` when the flag is
{cpp:enum}`false`, because there's no way for the window to cease being the active
...
{cpp:func}`~BWindowScreen::ScreenConnected` for accurate
notifications of when the {hclass}`BWindowScreen` gains and loses control of the
screen.
...
Overrides the {cpp:class}`BWindow` version of {cpp:func}`~BWindow::WorkspaceActivated` to connect the
{hclass}`BWindowScreen` object to the screen when the {hparam}`active` flag is {cpp:enum}`true` and to
disconnect it when the flag is {cpp:enum}`false`. User's typically activate the game
