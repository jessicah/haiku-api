...
Creates and returns a new {hclass}`BMessageFilter`. The first
...
-
	- delivery
	- Specifies how the message must arrive: drag-and-drop ({cpp:enum}`B_DROPPED_DELIVERY`), programmatically ({cpp:enum}`B_PROGRAMMED_DELIVERY`), or either ({cpp:enum}`B_ANY_DELIVERY`). The default is {cpp:enum}`B_ANY_DELIVERY`.
-
	- source
	- Specifes whether the sender of the message must be local vis-a-vis this app ({cpp:enum}`B_LOCAL_SOURCE`), remote ({cpp:enum}`B_REMOTE_SOURCE`), or either ({cpp:enum}`B_ANY_SOURCE`). The default is {cpp:enum}`B_ANY_SOURCE`.
-
	- command
...
The {hparam}`filter` argument is a pointer to a {cpp:func}`~filter::hook`
function. This is the function that's invoked when a message needs to be
examined (see
{cpp:func}`~filter::hook`
for the protocol). You don't have to supply a
{cpp:func}`~filter::hook`
function; instead, you can implement
{hclass}`BMessageFilter`'s
{cpp:func}`~BMessageFilter::Filter`
function in a subclass.

For more information, refer to the description of the member {cpp:func}`~BMessageFilter::Filter`
function.
...
{hparam}`message` that's being considered, and the proposed
{cpp:class}`BHandler`
{hparam}`target`. You can alter the contents of the message,
...
-
	...
	- Description
-
	- {cpp:enum}`B_DISPATCH_MESSAGE`.
	- The {hparam}`message` and {hparam}`handler` are passed (by the caller) to the looper's {cpp:func}`~BLooper::DispatchMessage` function.
-
	- {cpp:enum}`B_SKIP_MESSAGE`.
	- The message goes no further—it's immediately thrown away by the caller.
:::

The default version of this function returns {cpp:enum}`B_DISPATCH_MESSAGE`.

It's possible to call your {hmethod}`Filter()` function
...
{hclass}`BMessageFilter` with a
{cpp:func}`~filter::hook`
callback when you construct the object. If you do both, the
{cpp:func}`~filter::hook`
(and not {hmethod}`Filter()`) will be invoked when the object
...
{hmethod}`Command()` returns the command constant (the {cpp:class}`BMessage`
what value) that an arriving message must match for the
filter to apply. {hmethod}`FiltersAnyCommand()` returns
{cpp:enum}`true` if the filter applies to all messages, and
{cpp:enum}`false` if it's limited to a specific command.
...
{hmethod}`FiltersAnyCommand()` returns
{cpp:enum}`false`.
...
whose messages this object filters, or {cpp:enum}`NULL` if the
{hclass}`BMessageFilter` hasn't yet been assigned to a {cpp:class}`BHandler` or {cpp:class}`BLooper`. To attach a
{hclass}`BMessageFilter` to a looper or handler, use {cpp:func}`BLooper::AddCommonFilter`
or {cpp:func}`BHandler::AddFilter`.
...
{hclass}`BMessageFilter` object was constructed, that describe
the categories of messages that can be filtered.
{hmethod}`MessageDelivery()` returns a constant that
specifies how the message must be delivered
({cpp:enum}`B_DROPPED_DELIVERY`,
{cpp:enum}`B_PROGRAMMED_DELIVERY`, or
{cpp:enum}`B_ANY_DELIVERY`).
{hmethod}`MessageSource()` returns how the source of the
message is constrained ({cpp:enum}`B_LOCAL_SOURCE`,
{cpp:enum}`B_REMOTE_SOURCE`, or
{cpp:enum}`B_ANY_SOURCE`).
...
functions. The first two arguments are the {hparam}`message`
that's being considered, and the proposed {cpp:class}`BHandler` target. You can
...
{hparam}`messageFilter` is a pointer to the object on whose
behalf this function is being called; you mustn't delete this object. More
than one {hclass}`BMessageFilter` can use the same
...
You supply a {hclass}`BMessageFilter` with a
filter_hook function when you constuct the object.
Alternatively, you can subclass {hclass}`BMessageFilter` and
provide an implementation of {cpp:func}`~BMessageFilter::Filter`.
If you do both, the filter_hook (and not {cpp:func}`~BMessageFilter::Filter`)
...
See also:
The {hclass}`BMessageFilter`
{cpp:func}`~BMessageFilter::Constructor`
::::
