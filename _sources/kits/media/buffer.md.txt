...
it'll just alias an existing buffer.
::::
...
:::
The {hmethod}`Data()` function returns a
pointer to the first byte of the buffer, or
{cpp:enum}`NULL` if the buffer somehow couldn't be properly initialized (in which
...
({cpp:enum}`B_F1_BUFFER`), and other buffers are intended for
even video fields ({cpp:enum}`B_F2_BUFFER`), thereby letting
...
is available for use by other clients, {hmethod}`ID()` returns a positive buffer ID.
...
Don't call both
BroadcastBuffer()
and {hmethod}`Recycle()` on the same buffer.
:::
::::
...
:::
{hmethod}`SizeUsed()` returns the number of
bytes in the buffer that are currently in use.
{hmethod}`SetSizeUsed()` sets this value. This should be called after writing data
...
-
	...
	- Description
-
	- {cpp:enum}`B_F1_BUFFER`.
	- Buffer is for odd fields.
-
	- {cpp:enum}`B_F2_BUFFER`.
...
odd video fields; if {cpp:enum}`B_SMALL_BUFFER` is set, the buffer is a
...
:::
Describes where in memory a {hclass}`BBuffer`
