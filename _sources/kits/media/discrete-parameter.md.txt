...
:::
Adds a new {hparam}`name`/{hparam}`value`
...
:::{admonition} Note
:class: note
It's important to note that the correct inputs and outputs must already
be connected to the parameter when you call either of these functions,
...
:::
Returns {cpp:enum}`B_INT32_TYPE`, which is the data
type a {hclass}`BDiscreteParameter`'s value is maintained in.
::::
