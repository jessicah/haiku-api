...
{cpp:func}`~media::format` or
{cpp:func}`~media::codec`
form of the constructor, you must call
{cpp:func}`~BMediaEncoder::InitCheck`
...
information into the output file. {hparam}`code` indicates the type of information
being added, and {hparam}`data` is the information data itself.
{hparam}`size` indicates the
size of the data to be written.
By default, this function returns {cpp:enum}`B_ERROR`. Your implementation should
return {cpp:enum}`B_OK` if the information was written successfully; otherwise it
...
:::
Encodes a chunk of media data from the input buffer by {hparam}`inBuffer`, which
contains {hparam}`frameCount` frames of data.
The
{cpp:func}`~media::encode`
structure info is used on input to specify encoding parameters.
After the data is encoded, the encoder will call the derived class'
{cpp:func}`~BMediaEncoder::WriteChunk`
...
:::
{hmethod}`GetEncodeParameters()` returns in
{hparam}`inParameters` the
{cpp:func}`~encode::parameters`
...
any other {hclass}`BMediaEncoder` functions.
:::{list-table}
---
header-rows: 1
---
...
:::
{hmethod}`SetFormat()` sets the input and output formats to be used by the encoder,
as well as the
{cpp:func}`~media::file`
...
:::
{hmethod}`SetTo()` sets the format of media data
that will be encoded by the {hclass}`BMediaEncoder`
object. This also causes the {hclass}`BMediaEncoder` to
locate an appropriate codec to use.
The first form accepts a
{cpp:func}`~media::format`
structure, {hparam}`outputFormat`, that
...
-
	- Other Errors.
	- See {cpp:func}`~get::next` and {cpp:func}`~get::image`
:::
::::
...
of encoded data pointed to by {hparam}`chunkData` into the destination file or
buffer. {hparam}`chunkLength` indicates the size of the buffer, and info provides
information about the encoding process that you might need to reference.
Return {cpp:enum}`B_OK` if the data is successfully written; otherwise return an
