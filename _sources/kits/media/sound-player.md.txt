...
:::
Initializes the {hclass}`BSoundPlayer` object. The
{hparam}`name` argument specifies the name to be assigned
to the sound player node (if you specify {cpp:enum}`NULL`, a
generic name will be assigned).
The {hparam}`PlayBuffer` argument specifies a pointer to a member function that
...
The {hparam}`Notifier` parameter specifies a pointer to a member function that
receives notifications when events of interest occur, such as playback
starting or stopping. Specify {cpp:enum}`NULL` to use the default notification
handler. There are three possible notifications:
:::{list-table}
---
header-rows: 1
---
-
	...
	- Description
-
	- {cpp:enum}`B_STARTED`and{cpp:enum}`B_STOPPED`
	- Indicate that the {hclass}`BSoundPlayer` was started or stopped. These receive a pointer to the {hclass}`BSoundPlayer` object as an argument.
-
	- {cpp:enum}`B_SOUND_DONE`
	- Indicates that a sound has finished playing. In this case, there are two arguments: the first is a play_id indicating which sound finished playing, and the other is a boolean which is {cpp:enum}`true` if the sound played at all, and {cpp:enum}`false` if the sound never played.
:::
:::{admonition} Note
:class: note
...
:::
Stops playback, if sound is playing,
releases references to any BSound objects that are in use by the BSoundPlayer,
and frees all memory used by the {hclass}`BSoundPlayer`.
::::
...
:::
{hmethod}`BufferPlayer()` returns a pointer to the
current play buffer function, or {cpp:enum}`NULL` if the default
player is in use.
{hmethod}`SetBufferPlayer()` lets you change the play
buffer function.
::::
...
:::
{hmethod}`Cookie()` returns the current cookie
assigned to the {hclass}`BSoundPlayer`.
{hmethod}`SetCookie()` lets you change the cookie
assigned to the {hclass}`BSoundPlayer`.
::::
...
:::
{hmethod}`CurrentTime()` returns the current
media time, and {hmethod}`PerformanceTime()`
returns the current performance time of the sound player node being used
by the {hclass}`BSoundPlayer`.
{hmethod}`PerformanceTime()` will return
{cpp:enum}`B_ERROR` if the {hclass}`BSoundPlayer`
object hasn't been properly initialized.
::::
...
:::
{hmethod}`EventNotifier()` returns a
pointer to the current event notification
handler function, or {cpp:enum}`NULL` if the default player is in
use.
{hmethod}`SetNotifier()` lets you change the event
...
:::
Returns the {hclass}`BSoundPlayer`'s format. Since
...
{hclass}`BSoundPlayer`'s volume control. Pass pointers to
variables to be filled ({cpp:enum}`NULL` is not permitted),
...
-
	- outNode
	- Is the node to which the {hclass}`BSoundPlayer`'s audio output will be directed.
-
	- outParameter
...
:::
{hmethod}`HasData()` returns
{cpp:enum}`true` if there's sound queued for playback, or
{cpp:enum}`false` otherwise.
{hmethod}`SetHasData()` specifies whether or not
there's sound scheduled for playback.
The purpose of these functions is to optimize the
{hclass}`BSoundPlayer`; if there's no data queued for
...
{hmethod}`SetHasData()` to indicate that there's data to
play:
:::{code}
SetHasData(true);
:::
...
{hclass}`BSoundPlayer` object. You should call this after
constructing the object; if the returned value is anything other than
{cpp:enum}`B_OK`, you shouldn't use the object.
::::
...
:::
{hmethod}`Start()` activates the
{hclass}`BSoundPlayer` by starting the time source and the
sound player node. The {cpp:enum}`B_STARTED` notification is
sent to the {hclass}`BSoundPlayer`'s notification
handler.
{hmethod}`Stop()` deactivates the
{hclass}`BSoundPlayer` by stopping the player node (if
{hparam}`block` is {cpp:enum}`true`, the
{hmethod}`Stop()` function blocks until the node is
stopped). If {hparam}`flush` is
{cpp:enum}`true`, the queued sounds are all deleted from
memory.
While the {hclass}`BSoundPlayer` is running, the
play buffer function (if you've specified one) will be called for each
buffer that passes through the {hclass}`BSoundPlayer`'s
...
on-the-fly, or is filtered before playback.
:::{list-table}
---
header-rows: 1
---
...
-
	- Other errors.
	- As returned by the {hclass}`BMediaRoster`'s {cpp:func}`~BMediaRoster::StartNode` function.
:::
::::
...
:::
{hmethod}`StartPlaying()`
schedules the specified {hclass}`BSound` to begin playback
at the performance time specified by {hparam}`atTime`; if
{hparam}`atTime` is 0, the sound begins playing
immediately (or as soon as {hmethod}`Start()` is called,
if the {hclass}`BSoundPlayer` hasn't been started yet).
...
{hmethod}`StartPlaying()` lets you specify a volume at
which the sound should play.
{hmethod}`StopPlaying()` stops
playing the sound specified by the given
{hparam}`id`.
{hmethod}`WaitForSound()` waits
until the specified sound stops playing.
{hmethod}`IsPlaying()` returns
{cpp:enum}`true` if the specified sound is playing; otherwise,
it returns {cpp:enum}`false`.
:::{list-table}
---
header-rows: 1
---
...
-
	...
	- The specified id doesn't exist ({hmethod}`WaitForSound()` and {hmethod}`IsPlaying()`).
:::
::::
...
{hclass}`BSoundPlayer`.To control the volumes of individual sounds, use the SetSoundVolume() function.
:::{admonition} Note
:class: note
The volume can range from 0.0 to 1.0, where 0.0 is silent and 1.0 is
maximum loudness.
:::
If you'd rather handle the volume using decibels instead of the
percentage range, you can use the
{cpp:func}`~BSoundPlayer::VolumeDB` and
{cpp:func}`~BSoundPlayer::SetVolumeDB`
functions.
::::
...
{cpp:func}`~BSoundPlayer::SetSoundVolume`
function.
:::{admonition} Note
:class: note
...
{cpp:func}`~BSoundPlayer::Volume` and
{cpp:func}`~BSoundPlayer::SetVolume`
functions.
::::
...
{hclass}`BSoundPlayer`;
{cpp:func}`~BSoundPlayer::StartPlaying`
returns values of this type.
::::
