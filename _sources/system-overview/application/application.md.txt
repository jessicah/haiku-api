...
and set the cursor's image. See {cpp:func}`SetCursor()
<BApplication::SetCursor>`.

Access the window list.

: You can iterate through the windows that your application has created with
{cpp:func}`WindowAt() <BApplication::WindowAt>`.

Get information about your application.

: Your app's signature, executable location, and launch flags can be
retrieved through {cpp:func}`GetAppInfo() <BApplication::GetAppInfo>`.
Additional information icons, version strings, recognized file types can be
retrieved by creating an {cpp:class}`BAppFileInfo` object based on your
app's executable file. {cpp:class}`BAppFileInfo` is defined in the
{ref}`The Storage Kit`.
...
the {cpp:func}`Run() <BApplication::Run>` function. However, unlike other
loopers, {cpp:class}`BApplication`'s {cpp:func}`Run() <BApplication::Run>`
doesn't return until the application is told to quit. And after
{cpp:func}`Run() <BApplication::Run>` returns, you delete the object it
isn't deleted for you.
...
-
	...
	- The {hmethod}`main()` function doesn't declare {hparam}`argc` and
		...
		the {cpp:func}`ArgvReceived() <BApplication::ArgvReceived>` hook function.
-
	- {ref}``
...
-
	...
	- As explained in the {cpp:class}`BLooper` class, {cpp:func}`Run()
		<BApplication::Run>` is almost always called from the same thread in which
		you construct the {cpp:class}`BApplication` object. (More accurately, the
		constructor locks the object, and {cpp:func}`Run() <BApplication::Run>`
		unlocks it. Since locks are scoped to threads, the easiest thing to do is
		to construct and {cpp:func}`Run() <BApplication::Run>` in the same thread.)
...
{cpp:func}`MessageReceived() <BApplication::MessageReceived>` function (or
that of a designated {cpp:class}`BHandler`; for more on how messages are
dispatched to handlers, see ("{ref}`From Looper to Handler`").
...
functions are invoked by {cpp:func}`DispatchMessage()
<BApplication::DispatchMessage>` so the application messages never show up
in {cpp:func}`MessageReceived() <BApplication::MessageReceived>`.
...
-
	...
	- {cpp:func}`ArgvReceived() <BApplication::ArgvReceived>`
...
-
	...
	- {cpp:func}`RefsReceived() <BApplication::RefsReceived>`
...
-
	...
	- {cpp:func}`ReadyToRun() <BApplication::ReadyToRun>`

	- Invoked from within {cpp:func}`Run() <BApplication::Run>`, the application
...
-
	...
	- {cpp:func}`AppActivated() <BApplication::AppActivated>`
...
-
	...
	- {cpp:func}`Pulse() <BApplication::Pulse>`
...
-
	...
	- {cpp:func}`AboutRequested() <BApplication::AboutRequested>`
...
{cpp:class}`BApplication`'s implementation of {cpp:func}`Quit()
<BApplication::Quit>` is different from {cpp:class}`BLooper`'s version.
Don't miss it.
